...
model {
        ...
        offset_generation ~ normal(3449, 600);
    ...
    for (i in 1:N_samples) {
        ...
        generation_samples[i] ~ normal(a_ws * pow(wind_speed_historic[i], 2) + b_ws * wind_speed_historic[i] + offset_generation, 1000);
...
generated quantities {
    ...
    for (i in 1:N) {
        ...
        generation[i] = normal_rng(a_ws * pow(wind_speed[i], 2) + b_ws * wind_speed[i] + offset_generation, 1000);
        prices[i] = normal_rng(a_demand * demand[i] + a_generation * generation[i] + offset_prices, 100);
        log_lik[i] =  normal_lpdf(prices[i] | a_demand * demand[i] + a_generation * generation[i] + offset_prices, 100);
    }
}